...
# --- AYARLAR ---
ctk.set_appearance_mode("System") # Linux temasına uyması için System yaptık
...
class TPNApp(ctk.CTk):
    def __init__(self):
        ...
        self.title("TPN Hesaplayıcı (Linux Edition)")
        self.geometry("1100x850")
        
        self.latest_results = None
        
        # --- PATH BULMA (Linux/Windows Uyumlu) ---
        if getattr(sys, 'frozen', False):
            self.app_path = os.path.dirname(sys.executable)
        else:
            self.app_path = os.path.dirname(os.path.abspath(__file__))

        # ... (Geri kalan tüm kod yapısı Windows versiyonu ile birebir aynıdır) ...
        # (Kodun geri kalanını aynen buraya yapıştırabilirsin, 
        # sadece en üstteki satır ve app_path mantığı önemlidir.)
    ...
    def save_to_excel(self):
        ...
        # Linux'ta da çalışacak path birleştirme yöntemi
        file_name = os.path.join(self.app_path, "TPN_Kayitlari_Yatay.xlsx")
        
        keys = list(self.latest_results.keys())
        ...
        try:
            ...
            messagebox.showinfo("Başarılı", f"Kayıt Eklendi (Yatay)!\nDosya Yeri: {file_name}")
