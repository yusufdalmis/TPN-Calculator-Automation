...
from openpyxl.utils import get_column_letter # Sütun harfi bulmak için (A, B, C...)
...
class TPNApp(ctk.CTk):
    ...
    def save_to_excel(self):
        ...
        # Dosya adı: "Yatay" olarak değiştirdim ki eski dosya ile karışmasın
        ...
        try:
            if not os.path.exists(file_name):
                ...
                # A Sütununa Başlıkları Yaz
                for i, key in enumerate(keys, start=1):
                    ws.cell(row=i, column=1, value=key)
                ws.column_dimensions['A'].width = 25 # A sütununu genişlet
                
                # B Sütununa İlk Hasta Verisini Yaz
                ...
                # Dolu olan son sütunu bul ve bir yanına geç
                next_col = ws.max_column + 1
                
                # Verileri yeni sütuna işle
                for i, val in enumerate(values, start=1):
